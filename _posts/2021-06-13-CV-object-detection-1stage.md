...
上图的 Darknet-53 网络采用 `256 x 256 x 3` 作为输入，首先经过一个 `3 x 3 x 32` 卷积层得到，然后经过一个 `3 x 3 x 64s2` 卷积层下采样一倍。后面还可以看到， DarkNet 摒弃了所有最大池化层，转而用 stride=2 的卷积层来做下采样。比如第二层的下采样卷积：
...
后接一个 `3 x 3 x 256s2` 的下采样卷积。进入第二个残差模块，这里由 8 个残差模块堆叠而成。后面以此类推。
...
首先，主干网络最末端输出 `13 x 13 x 1024` ，经过 Convolutional Set，包含五个 `[卷积 + BN + Leaky ReLU]` 组（图右侧）， 卷积核具体为：

```
[1 x 1] x 512 --> [3 x 3] x 1024 --> [1 x 1] x 512 --> [3 x 3] x 1024 --> [1 x 1] x 512
```

再经过一个单独的 `[3 x 3 x 1024 卷积 + BN + Leaky ReLU]` 和一个 `1 x 1 x 255` 纯卷积，得到 `13 x 13 x 255` 的特征图。其中， 

```
255 = [ 80 class + 4 (x,y,w,h) + 1 confidence ] x 3 bbox
```

可识别 `13 x 13 = 169` 个大物体。

- **Predict Two**

具有中等尺度的感受野，用来检测中目标。主干网络最末端输出 `13 x 13 x 1024` ，经过五个 `[卷积 + BN + Leaky ReLU]` 组（图右侧），再经过一个单独的 `[1 x 1 卷积 + BN + Leaky ReLU]`，得到 `13 x 13 x 255`。然后经过上采样，得到输出维度 `26 x 26 x 255`。

经过上采样（up sampling）。常用方法包括：双线性插值（简单，不需要学习参数）和反卷积。

```
26 x 26 x 512 ------------------------------------------ 
                                                        | (concat)
13 x 13 x 255 -- (upsample) --> 32 x 32 x 512 --> 32 x 32 x 768
```
- **Predict Three**
...
32 x 32 x 256 ------------------------------------------ 
                                                        | (concat)
16 x 16 x 512 -- (upsample) --> 32 x 32 x 512 --> 32 x 32 x 768
```
