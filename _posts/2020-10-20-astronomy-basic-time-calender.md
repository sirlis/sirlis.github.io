...
- [2. 时间](#2-时间)
  - [2.1. 太阳时](#21-太阳时)
  - [2.2. 恒星时](#22-恒星时)
  - [2.3. 格林尼治标准时间](#23-格林尼治标准时间)
  - [2.4. 世界时（UT）](#24-世界时ut)
  - [2.5. 国际原子时（TAI）](#25-国际原子时tai)
  - [2.6. 协调世界时（UTC）](#26-协调世界时utc)
  - [2.7. 格林尼治标准时（GMT）](#27-格林尼治标准时gmt)
  - [2.8. 力学时（TT）](#28-力学时tt)
  - [2.9. 太阳系质心力学时（TDB）](#29-太阳系质心力学时tdb)
  - [2.10. 各时间的转换关系](#210-各时间的转换关系)
- [3. 历法](#3-历法)
...
历史上格林尼治标准时间的定义和计算较为混乱。比如，天文学领域常以正午12时作为格林尼治标准时间开始的计算方法，也有地方将其作为协调世界时UTC+0的别名。在英语人士中，格林尼治标准时间常被作为协调世界时的同义词，但实际上这是不正确的，因为格林尼治标准时间是一个具体时区（UTC+0），而不是一个时间标准。在导航领域，GMT常常被认为与UT1等同。正因为如此混乱的定义与使用，格林尼治标准时间不可以被单独作为精确的时间标准。
...
世界时（Universal Time，UT）是一种以地球自转为基础的时间计量系统，包含四个版本。所有四个版本都是基于地球相对遥远天体（恒星和类星体）的自转来定义的，只不过引入比例因子和其他调整使它们更接近太阳时。世界时曾长期被认为是稳定均匀的时间计量系统，历史上得到广泛应用。世界时的四个版本定义如下：

**UT0**：UT0是通过天文太观测恒星或河外射电源的日运动以及对月球和人造地球卫星的测距观测确定的世界时。天文台的位置在地球参考系（如国际地球参考系）中坐标固定。但地球自转轴的位置并不固定，而是在地球表面漂移，即极移。UT0不包含任何极移校正，并且已不再常用；

**UT1**：UT1是在UT0的基础上增加地球极移修正后得到的，是世界时的**主要形式**。与UT0类似，其本应该通过观测太阳的日运动来定义，但由于精确观测太阳十分困难，因此往往退而求其次，使用长基线干涉测量法确定遥远类星体位置、对月球和人造卫星进行激光测距、以及对GPS卫星进行轨道确定来计算；

**UT1R**：UT1R是在UT1的基础上增加周期性潮汐变化修正后得到的，它包括62个修正项，周期从5.6天到18.6年不等；

**UT2**：是在UT1的基础上增加季节性变化修正后得到的，已经不再常用。
...
国际原子时（Temps Atomique International，IAT）就是根据上述秒的定义确定的一种新的国际参照时标，其起点为1958年1月1日世界时0时，即在这一瞬间TAI和UT1重合（后来发现，当初那个瞬间二者并非完全重合，差了0.0039秒。这个差值就被作为事实保留下来。0.0）。目前，国际原子时由国际计量局收集50多个国400多个实验室的原子钟比对和时号发播资料进行综合处理后建立。
