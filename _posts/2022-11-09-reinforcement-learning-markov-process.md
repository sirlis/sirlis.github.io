...
**某状态 $s_t$ 的奖励 $R(s_t)$ 定义为从该状态过渡到所有可能状态的奖励的期望**。
...
从某个状态 $s_t$ 开始**一次采样**直到终止状态时（一条完整的状态序列）**所有奖励的折扣和**：
...
**价值函数**（Value）：对于马尔可夫奖励过程，价值函数被定义为**回报的期望**。也即从某个状态 $s_t$ 开始采样无数条完整状态序列后，其回报的平均值，即
...
但状态值函数也不好算，因为在计算某个状态时候需要使用到将来所有状态的 $G(s)$，这明显是不科学的。引入贝尔曼方程的目的是使状态值函数容易求解。
...
MDP 的状态价值函数（state-value Function），是从状态 $s$ 开始，执行策略 $\pi$ 所获得的回报的期望；或者说在执行当前策略时，衡量智能体处在状态 $s$ 时的价值大小。即
