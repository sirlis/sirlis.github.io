...
强化学习的最终目的，是**最大化智能体接收到的标量信号（收益）累积和的概率期望值** $G$
...
- $R$ 是奖励函数：$R_t = \mathbb{E}[R_{t+1}\vert S_t=s]$
...
某状态 $s_t$ 的奖励 $R_t$ 定义为**从该状态过渡到所有可能状态的奖励的期望**。
...
从四参数动态函数 $p$ 中，可以计算出关于环境的任何其它信息，如**状态转移概率**
