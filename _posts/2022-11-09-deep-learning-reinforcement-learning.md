...
回报值是针对一次完整的采样序列的结果，存在很大的样本偏差。即 $G_t$ 是从 $t$ 时刻的状态到终止状态的一条状态转移序列的回报值，但从 $t$ 时刻的状态到终止状态的马尔可夫链不止一条，每一条都有对应的概率和回报。对于复杂问题而言，要想精确的计算出 $G_t$ 是几乎不可能的，因为无法穷举所有序列。
...
状态值函数的引入解决了回报 $G_t$ 路径有很多条，不容易优化的问题，将其转化为期望，变成固定标量了。但状态值函数也不好算，因为在计算某个状态时候需要使用到将来所有状态的 $G_t$，这明显是不科学的。引入贝尔曼方程的目的是使状态值函数容易求解。
...
$$
\begin{aligned}
V_t &= \mathbb{E}[R_{t+1}+\gamma G_{t+1}\vert S_t=s_t]\\
&=R_{s}+\gamma \sum_{s_{t+1}\in S} V_{t+1}P(s_{t+1}\vert s_t)\\
\end{aligned}
$$
