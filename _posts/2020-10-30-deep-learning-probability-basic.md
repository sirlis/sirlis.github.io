...
一种**直观的理解**：假设 $B_1,...,B_n$ 是发生时间 $A$ 的各种原因，那么已知事件 $A$ 已经发生了，问是原因 $B_i$ 导致的概率 $P(B_i \vert A)$ 是多少？

另一种形式：

使用令一套字母体系：$H$（hypothesis）和 $E$（evidence），那么贝叶斯的推理过程可以表达为：通过不断收集证据 $E$ 来强化对假设时间 $H$ 的信心。那么贝叶斯公式可以描述为

$$
P(H\vert E) = \frac{P(H)P(E\vert H)}{P(E)}
$$

贝叶斯公式的思考模式意味着，新观察到的样本信息将修正人们以前对事物的认知。换言之，在得到新的样本信息之前，人们对 $\theta$ 的认知是先验分布 $\pi(\theta)$，在得到新的样本信息 $x$ 后，人们对 $\theta$ 的认知为 $\pi(\theta \vert x)$。

而后验分布 $\pi(\theta \vert x)$一般也认为是在给定样本 $x$ 的情况下 $\theta$ 的条件分布，使其达到最大的值称为最大后验概率估计。
