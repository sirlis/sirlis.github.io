...
- [1. 概率](#1-概率)
  ...
  - [1.4. 贝叶斯公式](#14-贝叶斯公式)
  - [1.5. 概率和统计](#15-概率和统计)
  - [1.6. 概率函数与似然函数](#16-概率函数与似然函数)
  - [1.7. 极大似然估计](#17-极大似然估计)
  - [1.8. 概率的两大学派](#18-概率的两大学派)
- [2. 参考文献](#2-参考文献)
...
托马斯·贝叶斯（Thomas Bayes）在世时，并不为当时的人们所熟知，很少发表论文或出版著作，与当时学术界的人沟通交流也很少，用现在的话来说，贝叶斯就是活生生一民间学术 “屌丝”，可这个 “屌丝” 最终发表了一篇名为《An essay towards solving a problem in the doctrine of chances》，翻译过来则是《机遇理论中一个问题的解》。这篇论文发表后，在当时并未产生多少影响，在20世纪后，这篇论文才逐渐被人们所重视。
...
- **贝叶斯派** 的观点则截然相反，他们认为参数 $\theta$ 是随机变量，而样本 $X$ 是固定的，由于样本是固定的，所以他们重点研究的是参数 $\theta$ 的分布。

相对来说，频率派的观点容易理解，所以下文重点阐述贝叶斯派的观点。贝叶斯派既然把 $\theta$ 看做是一个随机变量，所以要计算 $\theta$ 的分布，便得事先知道 $\theta$ 的无条件分布，即在有样本之前（或观察到 $X$ 之前）， $\theta$ 有着怎样的分布呢？
...
上述思考模式意味着，新观察到的样本信息将修正人们以前对事物的认知。换言之，在得到新的样本信息之前，人们对 $\theta$ 的认知是先验分布 $\pi(\theta)$，在得到新的样本信息 $x$ 后，人们对 $\theta$ 的认知为 $\pi(\theta \vert x)$。

而后验分布 $\pi(\theta \vert x)$一般也认为是在给定样本 $x$ 的情况下 $\theta$ 的条件分布，而使达到最大的值称为最大后验概率估计，类似于经典统计学中的极大似然估计。
