...
$$
H(X) = \mathbb E[-{\rm log} P(x_i)] = -\int_x P(x){\rm log} P(x) {\rm d}x
$$
...
相对熵，relative entropy = Kullback-Leibler divergence，用于衡量两个概率分布之间的差异。对于两个概率分布 $P(x)$ 和 $Q(x)$，其相对熵的计算公式为
...
当，$P(x) = Q(x)$时，$KL(P\Vert Q) = 0$。若 $P(x)$ 和 $Q(x)$ 略有差异，其值就会大于 0 。其证明利用了负指数函数是严格凸函数的性质。证明过程如下：

> 声时刻. [PRML书中公式（1.118）KL散度恒大于等于 0 的推导](https://blog.csdn.net/shenshikexmu/article/details/89185142)
