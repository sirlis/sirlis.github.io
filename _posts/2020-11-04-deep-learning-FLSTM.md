...
$x_{ij}=0$ 表示位置坐标位于网格 $j$ 中心，$x_{ij}$ 的取值上界可借助下图阐释，可以看出位置坐标距离网格中心最远不超过 $\sqrt 2 l$，即途中粉色方形的对角点距离。
...
借助模糊集的概念来定义模糊轨迹位置：$F\_Loc_{p_i} =N\_Loc_{p_i},M\_Loc_{p_i}$。其中 $N\_Loc_{p_i}$ 是位置坐标的临近网格集合，$M\_Loc_{p_i}$ 是相应的隶属度。
