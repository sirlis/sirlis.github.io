...
- 其次是一个简单的前向反馈神经网络 $g$。它由一个激活函数为 $tanh$ 的隐藏层和一个 $softmax$ 输出（分类）层组成，，可以将我们得到的一系列输出映射成对应概率。这样，通过将 Embedding 层输出的 $N−1$ 个词向量映射为一个长度为 $V$ 的概率分布向量，从而对词典中的 word 在输入 context 下的条件概率做出预估。
