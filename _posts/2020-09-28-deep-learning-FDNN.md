...
假设图像的类别为 $k=4$ 类，分别为 {**篮球**，**海滩**，**自行车**，**显示器**}，那么对于输入层的每个神经元（图像特征向量的每个元素），其针对每个类别均可以设计一个隶属度函数，那么隶属度函数总共为 $6\times 4$ 个，也即隶属度函数层的神经元个数为 $6\times 4$。
...
接着举例，依然是输入**自行车**的图像，其 6 个特征维度分别经过隶属度函数后，对应第三类的模糊度应该都很高。那么经过模糊规则层其连乘后，第三个神经元的取值相比其它神经元而言是一个大值（比如 $0.96\times 0.91\times 0.82\times 0.93\gg0.1\times0.2\times0.12\times0.23$）。反之，如果输入一个**手机**的图像，其在各方面特征都比较符合显示器的特征，那么可能在模糊规则层上第四个神经元的连乘结果较大，但因为它又不是一个显示器，那么可能其连乘结果可能又小于输入**显示器**图像时的结果，即 $0.76\times 0.82\times 0.89\times 0.75 < 0.89\times0.93\times0.88\times0.95$。
...
分别对模糊逻辑表示部分和神经网络部分（包括神经表示、混合、任务驱动部分）进行初始化。
...
对于模糊表示部分，所有层（即隶属度层和模糊规则层）的权值均为1。隶属度函数层的神经元节点还包括两个额外的未知参数：第 $i$ 个模糊神经元节点的模糊中心 $\mu_i$ 和模糊宽度 $\sigma_i$。作者根据输入数据的 k-均值聚类结果（k与分类数相等），采用下面参考文献中的方法初始化这些参数。
