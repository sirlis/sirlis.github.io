...
以某个例子为例，假设输入的图像是一个**自行车**，被转化为一组 **76** 维的特征向量，则输入层（Input layer）为 76 个神经元。假设图像的类别为 **4** 类，那么对于输入层的每个神经元（图像特征向量的每个元素），其针对每个类别均可以设计一个隶属度函数，那么隶属度函数总共为 $76\times 4$ 个，也即隶属度函数层的神经元个数为 $76\times 4$。

假设图像特征向量的第一个元素表征图像中**圆圈的个数**，图像的 4 个类别分别为 {篮球，海滩，自行车，显示器}。这 4 个类别中，该特征元素的期望取值为 $\{1,0.2,2,0\}$ （海滩可能有热气球因此设为 0.2）。那么这四个隶属度函数分别可能的形状为：
