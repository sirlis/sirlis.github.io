...
- $R$ 是一个奖励函数：$R_s = \mathbb{E}[R_{t+1}\vert S_t=s_i]$
...
注意，在马尔可夫奖励过程中，奖励指的是离开一个状态或者进入一个状态的奖励（用来评价状态的好坏）。比如说到达 $s_1=C_1$​ 状态的时候，可以获得 $-2$ 的奖励，到达 $s_4=Pass$​ 的时候，可以得到 $+10$ 的奖励，其它状态没有任何奖励。

**收益**（Return）：是一个马尔可夫奖励过程中，从某一个状态 $s_t$ 开始采样直到终止状态时所有奖励的折扣和：
...
当我们有了收益 Return 过后，就可以定义一个状态的价值了，就是状态价值函数（state value function）。对于马尔可夫奖励过程，状态价值函数被定义为 Return 的期望。

$$
V(s) = \mathbb{E}[G_t \vert s_{[t]}=s]
$$
