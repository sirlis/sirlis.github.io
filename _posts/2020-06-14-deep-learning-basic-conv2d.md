...
`softmax` 的计算方法如下，假设输入为一维向量 $Z = [z_1,z_2,...z_i,...,z_n]\in(-\infin,+\infin)$ ，则第 $i$ 个分量 $z_i$ 的 `softmax` 输出为
$$
...
我们知道指数函数 $e^x$ 的值域为 $(0,+\infin)$ ，因此 `softmax` 将 $(-\infin,+\infin)$ 的值映射到 $[0, 1]$。具体来说，`softmax` 具备下面两个作用
...
- $z_i - M$ 的取值范围为 $(-\infin,0]$ ，因此在做指数运算时输入的最大值为 0，从而避免上溢出。 
