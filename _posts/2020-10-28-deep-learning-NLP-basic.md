...
解码阶段可以看做编码的逆过程。这个阶段，我们要根据给定的语义向量 $H_{t-1}=[H_0=C,H_1,H_2,...,H_{t-1}]$ 和之前已经生成的输出序列 $y_1,y_2,...,y_{t−1}$ 来预测下一个输出的单词 $y_t$，而解码输出某个词 $y_t$ 出现的概率

$$
P(y_t)=p(y_t \vert \{y_1, y_2, ..., y_{t-1}\},H_{t-1})
$$

那么对于整个输出序列，我们需要使得实际输出与期望输出尽可能的吻合，问题就转化为

$$
{\rm max}_\theta P(y) = {\rm max}_\theta  \prod_{t=1}^n p(y_t \vert \{y_1, y_2, ..., y_{t-1}\},H_{t-1})
$$

这个式子是一个条件概率的连乘，$t$ 时刻输出的单词 $y_t$ 就是在 $1$ 到 $t-1$ 时刻的输出 $y_1,...,y_{t-1}$ 和语义向量 $H_{t-1}$ 一起作为条件的基础上得到的。上式表明，我们的目标是找到一组参数，使得从 $t=1$ 时刻到 $t=T$ 时刻输出的单词，与样本期望输出符合的概率最大。
